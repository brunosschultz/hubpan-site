...
function PerfilCard({ p }: { p: (typeof PERFIS)[number] }) {
  ...
  return (
    <div className="flex flex-col rounded-[20px] p-7" {...bgProps} style={{ background: bg }} data-animate>
      ...
      <div className="mt-auto"><Link to={p.to}><HubButton size="sm" variant={c.btn}><ET k={`casos.perfil.${p.id}.btn`} v={p.btn} l={`Casos de Uso — botão do perfil "${p.titulo}"`} /></HubButton></Link></div>
    </div>
  );
}
