...
const STATS: { value: number | string; prefix?: string; label: string; lime?: boolean }[] = [
  { value: 15, label: 'Edições realizadas' },
  { value: 10, label: 'Anos de trajetória' },
  { value: 4, label: 'Edições em Nova York' },
  { value: 6, label: 'Territórios estratégicos' },
  { value: 100, prefix: '+', label: 'Projetos abrigados' },
  { value: 'ONU', label: 'Presença institucional', lime: true },
];
...
const TERRITORIOS: Tile[] = [
  ...
  {
    kind: 'photocard', img: 's3-accordion-forum', tag: 'COOPERAÇÃO · MIPAD · FUTURO', nome: 'África',
  ...
  {
    kind: 'typo', color: 'navy', Icon: MapPin, img: 's6-numero-3', tag: 'ORIGEM · DESDE 2017', nome: 'Belo Horizonte',
...
const TIMELINE = [
  { ano: '2017', quando: 'Dezembro', titulo: '1ª edição EXPO BH — Belo Horizonte', major: true, desc: 'Marco zero. Sem precedentes no Brasil de conexão entre ODS e Smart Cities em um fórum estruturado.' },
  { ano: '2018', quando: '2018–2019', titulo: 'Expansão do Fórum Pan-Americano', major: false, desc: 'Consolidação do modelo em Belo Horizonte. Crescimento de participantes, projetos abrigados e relações institucionais.' },
  { ano: '2020', quando: '2020–2022', titulo: 'Extensão universitária e pós-graduação', major: false, desc: 'Primeiro ciclo de LTV real. Programas de extensão e pós-graduação lato sensu. Construção da carteira educacional que sustenta o PROINTER.' },
  { ano: '2023', quando: '2023', titulo: 'Harvard Square e MIT — Cambridge, MA', major: false, desc: 'Chegada ao ecossistema de inovação de Cambridge. Ancoragem como sede global.' },
  { ano: '2024', quando: '2024', titulo: '1ª entrada nas Nações Unidas · Nova York', major: true, desc: 'Primeiro contato real com o ambiente interno da ONU — mapeamento, relações e Top 100 da ONU.' },
  { ano: '2026', quando: 'Maio 2026', titulo: '1ª Expo Boston · Contrato do 4º andar da ONU', major: false, desc: 'Primeira edição fora do Brasil e de Nova York. Fechamento do contrato para ocupar o 4º andar inteiro das Nações Unidas.' },
  { ano: '2026', quando: 'Junho 2026', titulo: 'Lançamento do Portal HUB PAN', major: true, desc: 'O ecossistema se torna uma plataforma global. Portal, sede SP na Paulista, parceria MIPAD ONU e campanha do Fórum Mundial de IA.' },
  { ano: '2027', quando: 'Próximo capítulo', titulo: 'Fórum Mundial de Inteligência Artificial', major: true, desc: 'Cambridge, Massachusetts. O maior ativo proprietário do HUB PAN — autoridade global em IA em escala internacional.' },
];

const GOVERNANCA = [
  { Icon: Scale, titulo: 'Governança Institucional', desc: 'Estrutura formal de decisão, critérios de admissão de parceiros e prestação de contas públicos e rastreáveis.' },
  { Icon: GraduationCap, titulo: 'Governança Acadêmica', desc: 'Validação de programas educacionais, seleção de bolsistas do PROINTER e supervisão de conteúdo da Academy.' },
  { Icon: BrainCircuit, titulo: 'Governança de IA', desc: 'IA responsável centrada nas pessoas e alinhada aos ODS. Política de dados e transparência algorítmica.' },
  { Icon: Globe2, titulo: 'Governança de Programas', desc: 'Critérios de seleção, gestão de recursos e rastreabilidade de impacto para missões e fóruns internacionais.' },
  { Icon: ShieldCheck, titulo: 'Compliance e Integridade', desc: 'Alinhamento com marcos regulatórios nacionais e internacionais. Proteção de propriedade intelectual.' },
  { Icon: HeartHandshake, titulo: 'Princípios de Impacto', desc: 'ODS da ONU como critério de avaliação. Diversidade, equidade e inclusão como valores não negociáveis.' },
];

const MARCAS = ['Brasil Master® Group', 'Premier Niveau®', 'eGov Tecnologia®', 'EXPO BH®', 'EXPO NYC®'];

const FAQ = [
  { q: 'O HUB PAN é uma nova empresa ou uma nova marca?', a: 'Nem um nem outro exclusivamente. O HUB PAN é a nova narrativa global de um ecossistema que já existe há quase dez anos. As marcas operacionais continuam ativas (Brasil Master, Premier Niveau, eGov), mas o HUB PAN é a identidade unificadora que comunica a dimensão real do conjunto — para o mercado global.' },
  ...
  { q: 'O HUB PAN já tem histórico ou ainda está em construção?', a: 'O histórico é extenso e documentado: 15 edições do Fórum Pan-Americano da Inovação, 4 edições em Nova York, 1 edição em Boston, entrada na ONU, ancoragem em Harvard Square, centenas de projetos abrigados desde 2017. O HUB PAN é a nova narrativa desse histórico — não uma promessa vazia.' },
  { q: 'Como se conectar ao ecossistema?', a: 'Depende do seu perfil. Governos entram pelo GovIA. Empresas e investidores pelo Fórum Mundial de IA ou pela HUB PAN Alliance. Educadores e profissionais pelo PROINTER ou pela Academy. Qualquer perfil pode começar pelo formulário de contato institucional.' },
  { q: 'O que é a HUB PAN Alliance?', a: 'A Alliance é a rede estratégica de organizações do ecossistema — empresas, startups, universidades, ICTs e governos que fazem parte da comunidade HUB PAN com acesso a networking institucional, missões internacionais, visibilidade global e colaboração em projetos de impacto.' },
];
      ...
      <div className="w-full bg-navy900 border-t border-white/10 h-[20vh] min-h-[150px] flex items-center">
        <div className="gutter w-full">
          <div className="grid grid-cols-3 lg:grid-cols-6 gap-y-6">
            {STATS.map((s, i) => (
              <div key={s.label} className={`text-center px-2 ${i > 0 ? 'lg:border-l lg:border-white/10' : ''}`}>
  ...
  return (
    <section ref={ref} id="inst-manifesto" className="bg-white">
      <div
        ...
        style={{ background: 'linear-gradient(213deg, #ffffff 55%, #d2e718 100%)' }}
      >
        ...
        <div className="relative grid lg:grid-cols-2">
          ...
          <div className="order-1 lg:order-2 lg:h-screen flex flex-col justify-center gutter lg:pl-16 lg:pr-24 xl:pr-[160px] py-16 lg:py-0">
            <p className="eyebrow text-muted mb-7" data-animate>MANIFESTO FUNDACIONAL</p>
            ...
            <div data-animate>
              <Link to="/contato"><HubButton size="lg" variant="navy">Fale com nossa equipe</HubButton></Link>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <section ref={ref} className="lg:min-h-screen flex flex-col justify-center py-24 lg:py-32 gutter bg-white">
      <div className="mb-14 max-w-[640px]">
        <p className="eyebrow text-muted mb-6" data-animate>ECOSSISTEMA FUNDADOR</p>
        <h2 className="mb-4" style={{ fontFamily: 'Luxenta', fontWeight: 400, fontSize: 'clamp(32px,4vw,50px)', letterSpacing: '-0.5px', lineHeight: 1, color: '#152852' }} data-animate>
          De onde viemos define para onde vamos.
        </h2>
      ...
      <div className="grid md:grid-cols-3 gap-5 mb-5">
        {FUNDADORAS.map((f) => (
          <div key={f.nome} className="rounded-[20px] p-8 flex flex-col" style={{ background: f.bg, border: f.bg === '#fff' ? '1px solid #ecedf0' : 'none' }} data-animate>
            ...
            <p className="mb-2" style={{ fontFamily: 'Inter', fontWeight: 500, fontSize: 10.5, letterSpacing: '2px', textTransform: 'uppercase', color: f.tagColor }}>{f.tag}</p>
            <h3 className="mb-3" style={{ fontFamily: 'Luxenta', fontWeight: 600, fontSize: 24, color: f.text }}>{f.nome}</h3>
            <p className="mb-6" style={{ fontFamily: 'Inter', fontSize: 14, lineHeight: '23px', color: f.sub }}>{f.desc}</p>
    ...
    return (
      <div className={`flex flex-col rounded-[20px] overflow-hidden bg-white ${className}`} style={{ border: '1px solid #ecedf0' }} data-animate>
        <div className="relative overflow-hidden shrink-0 h-[55%] min-h-[220px]">
          <img src={`/images/${t.img}.webp`} alt={t.nome} className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" />
        </div>
        <div className="flex flex-col flex-1 p-7">
          <p className="mb-3" style={{ fontFamily: 'Inter', fontWeight: 500, fontSize: 10.5, letterSpacing: '1.8px', textTransform: 'uppercase', color: '#2d4ebf' }}>{t.tag}</p>
          <h3 className="mb-3" style={{ fontFamily: 'Luxenta', fontWeight: 600, fontSize: 24, lineHeight: 1.05, color: '#152852' }}>{t.nome}</h3>
          <p style={{ fontFamily: 'Inter', fontSize: 13.5, lineHeight: '21px', color: '#797979' }}>{t.desc}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <section ref={ref} className="py-24 lg:py-32 gutter" style={{ background: '#f5f5f5' }}>
      <div className="mb-14 max-w-[700px]">
        <p className="eyebrow text-muted mb-6" data-animate>PRESENÇA GLOBAL</p>
        <h2 className="mb-4" style={{ fontFamily: 'Luxenta', fontWeight: 400, fontSize: 'clamp(32px,4vw,50px)', letterSpacing: '-0.5px', lineHeight: 1, color: '#152852' }} data-animate>
          Onde o HUB PAN está — e o que cada território representa.
        </h2>
        <p style={{ fontFamily: 'Inter', fontSize: 16, lineHeight: '27px', color: '#797979' }} data-animate>
  ...
  return (
    <div ref={ref} className="relative pb-20 last:pb-0">
      ...
      <div
        ...
        data-animate
      >
        <h3 className="mb-2" style={{ fontFamily: 'Luxenta', fontWeight: 600, fontSize: 'clamp(18px,1.7vw,24px)', lineHeight: 1.1, color: '#152852' }}>{item.titulo}</h3>
        <p style={{ fontFamily: 'Inter', fontSize: 14, lineHeight: '23px', color: '#797979' }}>{item.desc}</p>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div ref={headRef} className="mb-16 max-w-[680px]">
        <p className="eyebrow text-muted mb-6" data-animate>10 ANOS DE TRAJETÓRIA</p>
        <h2 className="mb-4" style={{ fontFamily: 'Luxenta', fontWeight: 400, fontSize: 'clamp(32px,4vw,50px)', letterSpacing: '-0.5px', lineHeight: 1, color: '#152852' }} data-animate>
          Uma jornada construída entrega por entrega.
        </h2>
        <p style={{ fontFamily: 'Inter', fontSize: 16, lineHeight: '27px', color: '#797979' }} data-animate>
  ...
  return (
    <section ref={ref} className="relative w-full bg-navy900 overflow-hidden">
      ...
      <div className="relative gutter py-24 lg:py-32 lg:min-h-screen grid lg:grid-cols-2 gap-12 lg:gap-20 items-center content-center">
        <div>
          <p className="eyebrow mb-6" style={{ color: 'rgba(255,255,255,0.69)' }} data-animate>PARCEIRO ESTRATÉGICO FUNDACIONAL</p>
          ...
          >
            ...
            <span style={{ fontFamily: 'Inter', fontWeight: 500, fontSize: 13, color: '#fff' }}>Vinculado às Nações Unidas · ONU</span>
          </div>
          <div data-animate>
            <HubButton size="md" variant="lime">Saiba mais sobre o MIPAD</HubButton>
          </div>
        </div>
        ...
        >
          <div className="flex items-center justify-center py-10 mb-8 rounded-[14px]" style={{ background: 'rgba(255,255,255,0.04)' }}>
            <img src="/images/s10-logo-parceiro-1.png" alt="MIPAD" className="max-h-[110px] object-contain" />
          </div>
          <p className="mb-2" style={{ fontFamily: 'Inter', fontWeight: 500, fontSize: 10.5, letterSpacing: '2px', textTransform: 'uppercase', color: '#d2e718' }}>LANÇAMENTO DO PORTAL</p>
          <h3 className="mb-3" style={{ fontFamily: 'Luxenta', fontWeight: 600, fontSize: 22, color: '#fff' }}>CEO Global do MIPAD presente em São Paulo</h3>
  ...
  return (
    <section ref={ref} className="w-full bg-hubblue py-24 lg:py-32 gutter">
      <div className="text-center max-w-[640px] mx-auto mb-14">
        <p className="eyebrow mb-6" style={{ color: 'rgba(255,255,255,0.69)' }} data-animate>GOVERNANÇA GLOBAL</p>
      ...
      >
        <div>
          <div className="flex items-center gap-3 mb-5">
            ...
            <h3 style={{ fontFamily: 'Luxenta', fontWeight: 600, fontSize: 21, color: '#fff' }}>Marcas Registradas no INPI</h3>
          </div>
          <div>
            {MARCAS.map((m) => (
              <div key={m} className="flex items-center gap-3 py-[9px] border-b border-white/10 last:border-0">
                ...
                <span style={{ fontFamily: 'Inter', fontSize: 14, color: '#fff' }}>{m}</span>
              </div>
            ))}
          </div>
        </div>
        <div>
          <h3 className="mb-4" style={{ fontFamily: 'Luxenta', fontWeight: 600, fontSize: 21, color: '#fff' }}>Propriedade Intelectual</h3>
          ...
          <HubButton size="sm" variant="lime">Ver política de governança</HubButton>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section ref={ref} className="bg-white py-24 lg:py-32 gutter">
      <div className="grid lg:grid-cols-[1fr_2fr] gap-10 lg:gap-[72px] items-start">
        <div data-animate>
          <p className="eyebrow text-muted mb-6">PERGUNTAS FREQUENTES</p>
...
export default function Institucional() {
  return (
    <>
      ...
      <SecFAQ />
      <CTABanner
        title={<>Conecte-se ao futuro das <span style={{ color: '#d2e718' }}>Américas e da África.</span></>}
        sub="Seja como governo, empresa, educador, investidor ou comunidade — há um caminho para você no HUB PAN."
        actions={
          <>
            <Link to="/contato"><HubButton size="lg" variant="lime">Fale com nossa equipe</HubButton></Link>
            <Link to="/prointer"><HubButton size="lg" variant="navy">Conhecer o PROINTER</HubButton></Link>
          </>
        }
      />
    </>
