...
import { EImg, ERich, ET, useEditColors } from '../editor/fields';

interface City {
  id: string; img: string; name: string; label: string; sub: string; origin?: boolean;
}

const CITIES: City[] = [
  { id: 'bh', img: 's5-timeline-1', name: 'Belo Horizonte', label: 'ORIGEM | DESDE 2017', sub: 'Marco inicial', origin: true },
  { id: 'sp', img: 's5-timeline-2', name: 'São Paulo', label: 'SEDE BRASIL', sub: 'Av. Paulista' },
  { id: 'cambridge', img: 's5-timeline-3', name: 'Cambridge', label: 'SEDE GLOBAL', sub: 'Harvard Square' },
  { id: 'boston', img: 's5-timeline-4', name: 'Boston', label: 'INOVAÇÃO', sub: 'Ecossistemas Tech' },
  { id: 'ny', img: 's5-timeline-5', name: 'Nova York', label: 'DIPLOMACIA · ONU', sub: 'Reconhecimento' },
  { id: 'africa', img: 's5-timeline-6', name: 'Africa', label: 'COOPERAÇÃO', sub: 'Liderança - MIPAD' },
];
...
export default function S5Jornada() {
  ...
  const [[bgC1, bgC2], bgProps] = useEditColors('Fundo da seção Jornada', [
    { key: 's5.bg.c1', label: 'Cor principal', fallback: '#ffffff' },
    { key: 's5.bg.c2', label: 'Cor do degradê (canto)', fallback: '#d2e718' },
  ]);
        ...
        <div className="flex flex-wrap items-start justify-between gap-10 mb-16">
          <div className="max-w-[500px]">
            <p className="eyebrow text-muted mb-6" data-animate>
              <ET k="s5.eyebrow" v="JORNADA GLOBAL" l="Jornada — selo da seção" />
            </p>
            <h2 style={{ fontFamily: 'Luxenta', fontWeight: 400, fontSize: 'clamp(32px,4vw,50px)', lineHeight: '50px', color: '#152852' }} data-animate>
              <ERich k="s5.titulo" l="Jornada — título">De Belo Horizonte ao mundo.</ERich>
            </h2>
          </div>
          ...
          <div className="hidden xl:block relative shrink-0" style={{ width: 420, height: 200 }}>
            ...
            >
              <ET k="s5.ano" v="2017" l="Jornada — ano grande" />
            </span>
            <span
              className="absolute left-0"
              style={{ top: '32%', fontFamily: 'Luxenta', fontWeight: 400, fontSize: 40, color: '#152852' }}
            >
              <ET k="s5.desde" v="Desde" l="Jornada — palavra 'Desde'" />
            </span>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex items-center justify-center shrink-0 mb-4" style={{ width: CIRCLE_HOLDER, height: CIRCLE_HOLDER }}>
                    ...
                    >
                      <EImg
                        k={`s5.cidade.${c.id}.img`} v={`/images/${c.img}.webp`}
                        l={`Jornada — foto de ${c.name}`}
                        spec={{ w: 400, h: 400, shape: 'quadrada', note: 'Exibida em círculo — o centro da foto é o que aparece.' }}
                        alt={c.name}
